...
<body>
	<div class="container-fluid p-0" style="background-color: #EEEEEE">
		...
		<!-- 				Lịch SỬ GIAO DỊCH -->
		<div class="row m-3">
			<h1 class="text-primary text-center">Lich Sử Mua Hàng</h1>
				...
				<table class="table table-bordered">
					...
					<c:forEach var="item" items="${listOrder}">
						<tr>
							...
							<!-- 			USER      		 -->
							<c:if test="${sessionScope.account.role != true }">
								<c:if test="${item.isComfirm != true }">
									<th>
										<div class="rounded-circle bg-warning" style="width: 10px;height: 10px; display: inline-block;"> </div>
										<span> Đang Xác Nhận</span>
									</th>
								</c:if>
								<c:if test="${item.isComfirm == true && item.isPay != true }">
									<th>
										<div class="rounded-circle bg-primary" style="width: 10px;height: 10px; display: inline-block;"> </div>
										<span> Đang Giao</span>
									</th>
								</c:if>
								<c:if test="${item.isPay == true }">
									<th>
										<div class="rounded-circle bg-success" style="width: 10px;height: 10px; display: inline-block;"> </div>
										<span> Đã Nhận</span>
									</th>
								</c:if>
							</c:if>
							...
							<c:if test="${sessionScope.account.role == true }">
								<c:if test="${item.isComfirm != true }">
									<th class=" text-center">
										<form action="/home/order/admin/list-order/confirmOrder/${item.id }" method="Post">
											Đã Order -> <button type="submit" class=" btn btn-warning">Chờ Xác Nhận</button>
										</form>
									</th>
								</c:if>
								<c:if test="${item.isComfirm == true && item.isPay != true }">
									<th class=" text-center">
										<form action="/home/order/admin/list-order/payOrder/${item.id }" method="Post">
											Đã Xác Nhận -> <button type="submit" class=" btn btn-primary text-center">Giao Hàng</button>
										</form>
									</th>
								</c:if>
								<c:if test="${item.isComfirm == true && item.isPay == true }">
									<td class="text-center">
										<button type="submit" class=" btn btn-success text-center">Đã Thanh Toán</button>
									</td>
								</c:if>
							</c:if>
							<td>
								<a class="btn btn-warning" href="/home/order/showOrderByAccount/detailOrder/${item.id}">
									Chi Tiết
								</a>
							</td>
							<!-- ADMIN Hủy -->
							...
							<c:if test="${sessionScope.account.role == true }">
								<td class=" text-center">
									<a class=" btn btn-danger" href="/home/order/admin/list-order/delete?action=byadmin&id=${item.id }">Hủy</a>
								</td>
							</c:if>
							<!-- 		CUSTOMER Hủy 			-->
							<c:if test="${sessionScope.account.role == false && item.isPay != true }">
								<td class=" text-center">
									<a class=" btn btn-danger" href="/home/order/admin/list-order/delete?action=byuser&id=${item.id }">Hủy</a>
								</td>
							</c:if>
						</tr>
					</c:forEach>
