...
<body>
	<div class="container-fluid p-0" style="background-color: #EEEEEE">
		...
		<!-- 					END MENU							 -->
		<!--  -->
		<h2 class="text-center bg-light text-primary pt-3 border border-4 border-bottom-0 border-end-0 border-start-0 border-secondary"><u>Danh Sách Tài Khoản</u></h2>
		
		<!-- Message -->
		<c:if test="${not empty sessionScope.errorAcount }">
			<div class="alert alert-danger text-center">${sessionScope.errorAcount}</div>
			<c:remove var="errorAcount" scope="session"/>
		</c:if> 
		<div class="row m-3">
			<a class="btn btn-primary" href="/home/account/EditProfile?action=AddOrEdit" style="width: 100px">Add</a>
				...
				<c:forEach var="item" items="${listAccount}">
					<tr>
						<td class="text-center"><img
							...
							src="${pageContext.request.contextPath}/images/${item.photo }"
						...
						<td>${item.email }</td>
						<td>
							<c:if test="${item.role != true }">User</c:if>
							<c:if test="${item.role == true }">Admin</c:if>
						</td>
						<td><a class="btn btn-warning" href="/home/account/EditProfile?action=AddOrEdit&username=${item.username}">Update</a> 
							<a class="btn btn-danger" href="/home/account/EditProfile?action=Delete&username=${item.username}">Delete</a>
						</td>
					</tr>
				</c:forEach>
