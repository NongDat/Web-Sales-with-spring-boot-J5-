...
<body>
	<div class="container-fluid p-0" style="background-color: #EEEEEE">
		...
		<!-- 					Giỏ Hàng							 -->
		<div class="row mt-1">
			<h1 class="text-primary text-center bg-info pt-3 pb-3">Giỏ Hàng</h1>
			<article class="col-sm-7 offset-1">
				<div class="row">
					<table>
						<tr>
							<th>Tên Sản Phẩm</th>
						...
						<c:forEach var="item" items="${cartItems}">
							<form action="/shoppingCart/update" method="Post">
								<tr>
									<td>${item.name}</td>
									<td><a href="/home/product/detail/${item.id }"> <img class="border border-success rounded-circle"
									...
									<td><input type="number" value="${item.id}"
										name="productId" hidden="true" /> <input type="number"
										value="${item.quantity}" name="quantity"
										blur="this.form.submit()" />
										<button>Update</button> <a class="btn"
										href="/shoppingCart/delete/${item.id}">Delete</a> </input></td>
						...
						<tr class="">
							<td></td>
							<td class="" colspan=2><b>Tổng</b></td>
							<td class=""><b>${NoOfItems}</b></td>
						</tr>
			...
			<aside class="col-sm-2 offset-1">
				<div class="card mb-2">
					<div class="card-header">Tổng Tiền: <b> ${total}</b></div>
				</div>
				<div>
					<a class="btn btn-warning text-white btn-outline-primary" 
					href="/home/order/showOrder">Thanh Toán</a>
				</div>
			</aside>
		</div>
		<!-- 						FOOTER 									 -->
